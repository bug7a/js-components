...
JS Components Website - Code Highlight - v26.09
...
CodeHighlight.span = function (type, text) {
    return '<span class="wjc-' + type + '">' + CodeHighlight.escape(text) + '</span>';
};
...
CodeHighlight.injectCss = function () {
    ...
    if (document.getElementById("wjc-code-css")) return;

    let css = "";
    for (let type in CodeHighlight.COLORS) {
        css += ".wjc-" + type + "{color:" + CodeHighlight.COLORS[type] + ";}";
    }
    css += ".wjc-c{font-style:italic;}";

    const style = document.createElement("style");
    style.id = "wjc-code-css";
