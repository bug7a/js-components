...
basic.js Handbook Website - Code Highlight - v26.09 (copy of index/code-highlight.js (repo root))
...
CodeHighlight.span = function (type, text) {
    return '<span class="hb-' + type + '">' + CodeHighlight.escape(text) + '</span>';
};

CodeHighlight.highlight = function (text, language) {
    return (language == "html") ? CodeHighlight.html(text) : CodeHighlight.js(text);
};
...
CodeHighlight.injectCss = function () {
    ...
    if (document.getElementById("hb-code-css")) return;

    let css = "";
    for (let type in CodeHighlight.COLORS) {
        css += ".hb-" + type + "{color:" + CodeHighlight.COLORS[type] + ";}";
    }
    css += ".hb-c{font-style:italic;}";

    const style = document.createElement("style");
    style.id = "hb-code-css";
